    ...
    def forward(self, user, item):
        ...
        for i, key in enumerate(self.relation_dict):
            ...
            for j in range(self.K):
                if j == 0:
                    tmp_r_sum = torch.exp(
                        self.user_embedding[..., j * self.K:(j + 1) * self.K] @ s_i[j * self.K:(j + 1) * self.K, ...])
                else:
                    tmp_r_sum = tmp_r_sum + torch.exp(
                        self.user_embedding[..., j * self.K:(j + 1) * self.K] @ s_i[j * self.K:(j + 1) * self.K, ...])
            for j in range(self.K):
                tmp_r = torch.exp(
                    self.user_embedding[..., j * self.K:(j + 1) * self.K] @ s_i[j * self.K:(j + 1) * self.K, ...]) / tmp_r_sum
                s_u[..., j*self.K:(j+1)*self.K] = s_u[..., j*self.K:(j+1)*self.K]+tmp_r*tmp_relation_matrix.to_dense()@s_i[j*self.K:(j+1)*self.K, ...].t()

            for j in range(self.K):
                if j == 0:
                    tmp_user_neighbour = nn.functional.normalize(s_u[..., j * self.K:(j + 1) * self.K], dim=1)
                else:
    ...
    def evaluate(self, user):
        ...
        for i, key in enumerate(self.relation_dict):
            ...
            for j in range(self.K):
                tmp_r = torch.exp(
                    self.user_embedding[..., j * self.K:(j + 1) * self.K] @ s_i[j * self.K:(j + 1) * self.K, ...]) / tmp_r_sum
                s_u[..., j*self.K:(j+1)*self.K] = s_u[..., j*self.K:(j+1)*self.K]+tmp_r*self.relation_dict[key].to_dense()@s_i[j*self.K:(j+1)*self.K, ...].t()
